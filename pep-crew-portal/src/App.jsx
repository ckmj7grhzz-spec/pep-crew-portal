...
function buildCrewRowFromResourceBooking(booking, staffMembers = []) {
  ...
  const name = resource?.name || booking?.booking_name || 'Booked crew'
  ...
  return {
    ...
    hotel: '',
    room_number: '',
    notes: booking?.notes || staffMember?.notes || staffMember?.skills || '',
...
function mergeCrewRowsWithResourceBookings(crewRows = [], resourceBookings = [], staffMembers = []) {
  const existingNames = new Set((crewRows || []).map(member => normalisePersonName(member.name)).filter(Boolean))
  const existingEmails = new Set((crewRows || []).map(member => normalisePersonName(member.email)).filter(Boolean))

  const bookedCrewRows = (resourceBookings || [])
    ...
    .filter(member => {
      const nameKey = normalisePersonName(member.name)
      const emailKey = normalisePersonName(member.email)
  ...
  return [
    ...(crewRows || []).map(member => ({ ...member, source: 'crew_table' })),
    ...bookedCrewRows,
  ]
}
...
function AdminPage() {
  ...
  function getDashboardConflictItems() {
    const seen = new Set()
    const items = []
        ...
        getResourceConflicts(booking.resource_calendar_id, booking.start_date, booking.end_date || booking.start_date, booking.event_id)
          .forEach(conflict => {
            ...
            items.push({
              ...
              booking,
              conflict,
              primaryName: linkedEvent?.show_name || booking.booking_name || booking.title || 'Existing booking',
              conflictName: conflict.linked_event?.show_name || conflict.booking_name || conflict.title || 'Existing booking',
            })
          })
      })
  ...
  function getMissingInformationItems() {
    return inProgressCrewSheets.map(eventRecord => ({
      id: eventRecord.id,
      title: eventRecord.show_name || 'Untitled crew sheet',
      venue: eventRecord.venue || 'Venue TBC',
      href: getEventAdminHref(eventRecord),
      status: getCrewSheetStatusLabel(getCrewSheetStatus(eventRecord)),
    }))
  }

  function goToPortalSection(tab, targetId = null, options = {}) {
    setActivePortalTab(tab)

    if (options.resourceGroup) {
      setOpenResourceCalendarGroups(current => ({
        ...current,
        [options.resourceGroup]: true,
      }))
    }

    if (options.staffGroup) {
      setOpenStaffGroups(current => ({
        ...current,
        [options.staffGroup]: true,
      }))
    }
  ...
  function getResourceCategoryLiveBookings(category) {
    const today = formatCalendarDateInput(new Date())
    ...
    return (resourceBookings || [])
      ...
      .map(booking => ({
        ...
        linked_event: booking.linked_event || findLinkedEventForBooking(booking),
      }))
  ...
  function renderResourceCategorySummary(category, title, emptyText) {
    ...
    return (
      <section className="eventCard resourceCategorySummaryCard" id={`resource-section-${category}`}>
        <div className="staffSectionHeader">
          <div>
            <p className="eyebrowDark">Resource Overview</p>
            <h2>{title}</h2>
            <p>{liveBookings.length} live booking{liveBookings.length === 1 ? '' : 's'} today · {resources.length} resource{resources.length === 1 ? '' : 's'} configured.</p>
          </div>
        </div>

        {liveBookings.length ? (
          <div className="resourceSummaryList">
            {liveBookings.map(booking => (
              <div className="resourceSummaryItem" key={booking.id}>
                <div>
                  <strong>{booking.resource_calendar?.name || booking.booking_name || 'Resource booking'}</strong>
                  <span>{booking.linked_event?.show_name || booking.booking_name || 'Unlinked booking'}</span>
                </div>
                <small>{formatDate(booking.start_date)} → {formatDate(booking.end_date || booking.start_date)}</small>
              </div>
            ))}
          </div>
        ) : (
  ...
  function renderOperationsCalendar() {
    ...
    return (
      <>
        ...
        <section className="eventCard operationsCalendarCard" id="operations-calendar-main">
        ...
        <section className="eventCard operationsResourcesHub" id="operations-resources-hub">
          <p className="eyebrowDark">Operations Resources</p>
          <h2>Resource Sections</h2>
          <p>Dashboard cards jump here for vehicles, LED trailers and dry hire.</p>
        </section>
        {renderResourceCategorySummary('vehicles', 'Vehicles Out', 'No vehicles are booked out today.')}
        {renderResourceCategorySummary('led_trailers', 'LED Trailers Out', 'No LED trailers are booked out today.')}
        {renderResourceCategorySummary('dry_hire', 'Dry Hire Jobs', 'No dry hire resources are booked today.')}
      </>
    )
  }
    ...
    <main className="page adminPortalPage">
      ...
      {activePortalTab === 'staff' && (
        <>
          ...
          <section className="eventCard staffLeaveCard" id="staff-leave-section">
            <div className="staffSectionHeader">
              <div>
          ...
          <section className="eventCard staffDirectoryCard" id="staff-directory-section">
            <div className="staffSectionHeader">
              <div>
      ...
      {activePortalTab === 'reports' && (
        <>
          <section className="eventCard reportsOverviewCard">
            <p className="eyebrowDark">Reports</p>
            <h2>Operations Reports</h2>
            <p>Review readiness, completed projects, missing information and active warnings.</p>

            <div className="portalPlaceholderGrid reportsSummaryGrid">
              <div>
              ...
              <div className={getDashboardConflictCount() ? 'statusRed' : ''}>
                <strong>{getDashboardConflictCount()}</strong>
                <span>Conflict Warnings</span>
              </div>
              <div className={inProgressCrewSheets.length ? 'statusOrange' : ''}>
                <strong>{inProgressCrewSheets.length}</strong>
                <span>Missing Info Items</span>
              </div>
            </div>
          </section>

          <section className="eventCard reportsSectionCard" id="issues-warnings-report">
            <p className="eyebrowDark">Issues & Warnings</p>
            <h2>Conflicts and Missing Information</h2>
            <p>Active double-booking conflicts and crew sheets that still need attention.</p>

            <div className="reportsTwoColumnGrid">
              <div className="reportPanel warningReportPanel">
                <h3>Conflict Warnings</h3>
                {getDashboardConflictItems().length ? (
                  <div className="reportIssueList">
                    {getDashboardConflictItems().map(item => (
                      <div className="reportIssueItem" key={item.key}>
                        <strong>{item.resource?.name || 'Resource conflict'}</strong>
                        <span>{item.primaryName} clashes with {item.conflictName}</span>
                        <small>{formatDate(item.booking.start_date)} → {formatDate(item.booking.end_date || item.booking.start_date)}</small>
                      </div>
                    ))}
                  </div>
                ) : (
                  <Empty text="No resource conflicts detected." />
                )}
              </div>

              <div className="reportPanel missingReportPanel">
                <h3>Missing Information</h3>
                {getMissingInformationItems().length ? (
                  <div className="reportIssueList">
                    {getMissingInformationItems().map(item => (
                      <a className="reportIssueItem" href={item.href} key={item.id}>
                        <strong>{item.title}</strong>
                  ...
                  <Empty text="No in-progress crew sheets need attention." />
                )}
              </div>
            </div>
          </section>
            ...
            {completedCrewSheets.length ? (
              <div className="calendarEventList">
                {completedCrewSheets.map(eventRecord => (
                  <a className="calendarEventListItem" href={getEventAdminHref(eventRecord)} key={eventRecord.id}>
                    <div>
                      <strong>{eventRecord.show_name}</strong>
                      <span>{eventRecord.venue || 'Venue TBC'}</span>
                    </div>
...
function EventManagerPage() {
  ...
  async function loadEventManager() {
    ...
    const { data: hotelDirectoryData, error: hotelDirectoryError } = await supabase
  ...
  function applyHotelDirectoryToBulk(directoryId) {
    const selectedHotel = hotelDirectory.find(item => String(item.id) === String(directoryId))

    setBulkHotelForm(current => ({
      ...current,
      directory_id: directoryId,
      hotel_name: selectedHotel?.hotel_name || current.hotel_name,
      address: selectedHotel?.address || current.address,
      maps_url: selectedHotel?.maps_url || current.maps_url,
      what3words: selectedHotel?.what3words || current.what3words,
      hotel_contact: selectedHotel?.hotel_contact || current.hotel_contact,
      notes: selectedHotel?.notes || current.notes,
    }))
  }

  function applyHotelDirectoryToSingle(directoryId) {
    const selectedHotel = hotelDirectory.find(item => String(item.id) === String(directoryId))
    ...
    setHotelForm(current => ({
      ...
      what3words: selectedHotel.what3words || current.what3words,
      hotel_contact: selectedHotel.hotel_contact || current.hotel_contact,
      notes: current.notes || selectedHotel.notes || '',
    }))
  }

  function toggleBulkHotelCrew(crewName) {
    setBulkHotelForm(current => {
  ...
  async function saveBulkHotelToDirectory() {
    if (!bulkHotelForm.hotel_name.trim()) return
    ...
    const { error } = await supabase
      .from('hotel_directory')
      .upsert([payload], { onConflict: 'hotel_name' })
  ...
  async function saveEventResourceBooking(e) {
    ...
    await ensureCrewRowForBookedResource(event.id, resource, staffMembers, eventResourceBookingForm.notes)
  ...
  function getEventAssignedResources() {
    return resourceBookings
      .map(booking => {
        const resource = getEventResourceById(booking.resource_calendar_id)
        return {
          ...booking,
          resource_calendar: resource,
          staff_member: findStaffMemberForResource(resource, staffMembers),
        }
      })
  ...
  async function saveBulkHotel(e) {
    ...
    } else if (bulkHotelForm.apply_to === 'selected_crew') {
      const selectedNames = new Set(bulkHotelForm.selected_crew.map(normalisePersonName))
    ...
    if (bulkHotelForm.save_to_directory) await saveBulkHotelToDirectory()
    ...
    <main className="page">
      ...
      <section className="eventCard compactControlCard">
        <div className="statsGrid">
          <div>
            <strong>{combinedCrewRows.length}</strong>
            <span>Crew</span>
          </div>
          ...
          <div className="adminList">
            {filteredCrew.map(member => (
              <div className="adminListItem" key={member.id}>
                <div>
                  <strong>{member.name}</strong>
                  <p>{member.role || 'Crew'} {member.department && `| ${member.department}`}</p>
                  ...
                  {member.source === 'resource_booking' ? (
                    <>
                      <button type="button" onClick={() => {
                        ...
                        setEditingCrewId(null)
                        scrollToForm('crew-form')
                      }}>Edit</button>
                      <button type="button" onClick={() => deleteEventResourceBooking(member.resource_booking_id)}>Delete</button>
                    </>
                  ) : (
                    <>
      ...
      <section className="eventCard hotelAssignmentCard" id="hotel-form">
        <div className="bulkAssignHeader">
          <div>
            <p className="eyebrowDark">Hotel Assignment</p>
            <h2>{editingHotelId ? 'Edit Hotel Booking' : 'Add / Apply Hotel'}</h2>
            <p>Assign one guest manually or apply the same hotel to multiple crew from one place.</p>
          </div>
          <div className="assignmentModeToggle">
            <button
              type="button"
              className={hotelAssignmentMode === 'single' ? 'active' : ''}
              onClick={() => setHotelAssignmentMode('single')}
            >
              Single Guest
            </button>
            <button
              type="button"
              className={hotelAssignmentMode === 'bulk' ? 'active' : ''}
              onClick={() => setHotelAssignmentMode('bulk')}
            >
              Bulk Apply
            </button>
          </div>
        ...
        {hotelAssignmentMode === 'bulk' ? (
          <form onSubmit={saveBulkHotel} className="adminForm bulkAssignForm">
            <label>
              Hotel Directory
              <select value={bulkHotelForm.directory_id} onChange={e => applyHotelDirectoryToBulk(e.target.value)}>
                <option value="">Select saved hotel or enter new details</option>
                {hotelDirectory.map(hotel => (
            ...
            {bulkHotelForm.apply_to === 'selected_crew' && (
              <div className="crewSelectionPanel fullWidthFormItem">
                <strong>Select Crew</strong>
                <p>Choose exactly who should receive this hotel booking.</p>
                <div className="crewSelectionGrid">
                  {combinedCrewRows.map(member => {
                    const hasHotel = hotels.some(hotel => normalisePersonName(hotel.guest_name) === normalisePersonName(member.name))
                    const notRequired = member.hotel_required === false
                    return (
                      <label key={member.id} className={bulkHotelForm.selected_crew.includes(member.name) ? 'selected' : ''}>
                        <input
                          type="checkbox"
                          checked={bulkHotelForm.selected_crew.includes(member.name)}
                          onChange={() => toggleBulkHotelCrew(member.name)}
                        />
                        <span>
                          <strong>{member.name}</strong>
                          <small>{notRequired ? 'Hotel not required' : hasHotel ? 'Hotel already assigned' : 'Needs hotel'}</small>
                        </span>
                      </label>
                    )
                  })}
            ...
            <label className="checkboxRow bulkOverwriteToggle">
              <input
                type="checkbox"
                checked={bulkHotelForm.save_to_directory}
                onChange={e => updateBulkHotelField('save_to_directory', e.target.checked)}
              />
              Save this hotel to the directory
            </label>

            <div className="bulkAssignActions fullWidthFormItem">
              <button className="primaryButton" type="submit">
                <Plus size={18} /> Apply Hotel
              </button>
          ...
          <form onSubmit={saveHotel} className="adminForm">
            {editingHotelId && <p className="editNotice fullWidthFormItem">Editing: {hotelForm.guest_name} {hotelForm.hotel_name && `- ${hotelForm.hotel_name}`}</p>}

            <label>
              Hotel Directory
              <select value="" onChange={e => applyHotelDirectoryToSingle(e.target.value)}>
                <option value="">Select saved hotel or enter new details</option>
                {hotelDirectory.map(hotel => (
                  <option key={hotel.id} value={hotel.id}>{hotel.hotel_name}</option>
                ))}
...
function PublicCrewSheet() {
  ...
  const publicResourceBookings = (data.resource_bookings || [])
    .map(booking => {
      const resource = publicResourceCalendarById[String(booking.resource_calendar_id)]
      return {
        ...booking,
        resource_calendar: resource,
        staff_member: findStaffMemberForResource(resource, data.staff_members || []),
      }
    })
    .filter(booking => booking.resource_calendar)
  ...
  const publicCrewRows = mergeCrewRowsWithResourceBookings(data.crew || [], publicCrewResourceBookings, data.staff_members || [])
